...
- **[로직 12] 스케줄 자동 생성 로직** 🆕 - 계약서 기반 자동 스케줄 생성
...
### 📁 **관련 파일**
- `services/scheduleService.ts` (스케줄 서비스)
- `components/admin/modals/contract-form-modal.tsx` (계약서 작성 모달)
- `components/employee/qr-scanner.tsx` (QR 스캔 출퇴근)
- `lib/types/schedule.ts` (타입 정의)

### 📝 **목적**
계약서 작성 시 자동으로 한 달치 스케줄을 생성하고, 실시간 출퇴근 기록을 반영하여 관리자와 직원 모두에게 스케줄 정보를 제공

### 🎯 **핵심 기능**

#### 1. 계약서 → 스케줄 자동 생성
```typescript
// services/scheduleService.ts - generateMonthlySchedules()

// 입력:
// - contract: Contract (계약서 데이터)
// - creatorUid: string (생성자 UID)

// 처리:
// 1. 계약 시작월의 1일~말일 날짜 생성
// 2. contract.schedules 배열에서 요일별 근무시간 매핑
// 3. 각 날짜에 대해:
//    - 해당 요일에 근무가 있으면 PlannedTime 생성
//    - 신규 계약: schedules 컬렉션에 새로 생성 (덮어쓰기)
//    - 추가 계약: 기존 plannedTimes 배열에 추가 (병합)
// 4. Firestore batch로 최대 500개씩 커밋

// 출력:
// - schedules 컬렉션에 문서 생성 (ID: {userId}_{date})
```
...
// 예시: 같은 날 여러 시간대 근무
// 
// 계약서 1 (정규): 월~금 09:00-18:00
// 계약서 2 (추가): 월,수 19:00-22:00
// 
// 결과 (월요일):
// plannedTimes: [
//   { contractId: 'C1', startTime: '09:00', endTime: '18:00', isAdditional: false },
//   { contractId: 'C2', startTime: '19:00', endTime: '22:00', isAdditional: true }
// ]
```

#### 3. 실시간 출퇴근 반영
```typescript
// components/employee/qr-scanner.tsx + services/scheduleService.ts
// 
// QR 스캔 출퇴근 시:
// 1. attendance 컬렉션에 기록 저장
// 2. updateScheduleActualTime() 호출
// 3. schedules 컬렉션의 actualTime 필드 업데이트
//    - clockIn, clockOut (실제 시간)
//    - status (late, on_time, early_leave, overtime)
//    - warning, warningReason (경고 메시지)
```
...
#### **1. 문서 ID 형식**
```typescript
scheduleId = `${userId}_{date}`  // 예: "abc123_2025-12-30"
```
- 같은 직원의 같은 날짜는 1개 문서만 존재
- 추가 계약서는 plannedTimes 배열에 추가

#### **2. 한 달 단위 생성**
- 계약 시작월만 자동 생성
- 다음 달은 관리자가 수동으로 생성하거나 매월 1일 자동 실행 (추후)

#### **3. 계약서 수정 시 업데이트**
```typescript
// services/scheduleService.ts - updateSchedulesFromContract()
// 
// - 오늘 이후의 스케줄만 업데이트
// - 과거 스케줄은 보존 (이력 유지)
// - plannedTimes 배열에서 해당 contractId만 수정
```

#### **4. Firestore Rules 권한**
```javascript
// firestore.rules
// 
// schedules 컬렉션:
// - 읽기: 본인 or 같은 회사 직원 (매장 전체 보기)
// - 쓰기: store_manager 이상
```
...
```typescript
{
  ...
  plannedTimes: [
    {
      ...
      startTime: "09:00",
      endTime: "18:00",
      breakTime: { minutes: 60, isPaid: false },
      workHours: 8.0
    }
  ],
  ...
  // 실제 시간 (출퇴근 기록)
  actualTime: {
    clockIn: "09:10",      // 10분 지각
    clockOut: "19:30",     // 1.5시간 연장
    attendanceId: "abc123_2025-12-30_09-10-00",
    status: "late",
    warning: "⚠️ 지각: 10분 늦음",
    warningReason: "지각"
  },
...
계약서 작성 및 저장
  ↓
generateMonthlySchedules(contract, adminUid)
  ↓
계약 시작월의 1일~말일 날짜 생성
  ↓
contract.schedules 배열에서 요일별 매칭
  ↓
각 날짜에 PlannedTime 생성
  ↓
신규 계약: schedules에 새 문서 생성
추가 계약: 기존 plannedTimes에 추가
  ↓
Firestore batch 커밋 (최대 500개)
  ↓
관리자/직원 페이지에서 스케줄 조회
```
...
1. **Batch 제한**: Firestore batch는 최대 500개까지
   - 한 달이 31일이면 500개 문제 없음
   - 만약 여러 직원을 동시에 생성하면 batch 분할 필요

2. **타임존 처리**: 날짜는 YYYY-MM-DD 문자열로 저장
   - 시간은 HH:mm 문자열로 저장
   - Timestamp는 createdAt, updatedAt만 사용

3. **Legacy 호환**: `startTime`, `endTime` 필드는 Deprecated
   - 새 코드는 `plannedTimes` 배열 사용
   - 기존 데이터는 호환성 유지
