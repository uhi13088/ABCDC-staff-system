 ...
 * 
 * 사용 방법:
 * 1. admin-dashboard.html을 브라우저에서 열기
 * 2. 관리자로 로그인
 * 3. F12 → Console 열기
 * 4. 이 파일의 내용을 전체 복사
 * 5. 콘솔에 붙여넣고 Enter
...
async function checkStructure() {
  logMigration('\n========================================');
  logMigration('0단계: 현재 데이터 구조 확인');
  logMigration('========================================\n');
  
  const snapshot = await db.collection('schedules').limit(10).get();
  
  if (snapshot.empty) {
    logMigration('⚠️ schedules 컬렉션이 비어있습니다.', 'warning');
  ...
  logMigration(`샘플 문서 ID: ${firstDoc.id}`);
  logMigration(`샘플 데이터 구조: ${JSON.stringify(Object.keys(data))}`);
  
  // 주차별 구조 확인 (userId_year-week 형식)
  if (firstDoc.id.includes('_') && firstDoc.id.includes('-')) {
    const parts = firstDoc.id.split('_');
    if (parts.length === 2 && parts[1].includes('-')) {
      logMigration('✅ 주차별 구조 감지 - 마이그레이션 필요', 'success');
  ...
  if (data.date && typeof data.date === 'string' && data.date.match(/^\d{4}-\d{2}-\d{2}$/)) {
    logMigration('✅ 이미 날짜별 구조입니다 - 마이그레이션 불필요', 'success');
    return { needsMigration: false, type: 'daily' };
  }
  
  logMigration('⚠️ 알 수 없는 구조입니다.', 'warning');
...
async function backupData() {
  logMigration('\n========================================');
  logMigration('1단계: 데이터 백업');
  logMigration('========================================\n');
  
  const snapshot = await db.collection('schedules').get();
  logMigration(`백업할 문서: ${snapshot.size}개`);
  
  if (snapshot.empty) {
    logMigration('⚠️ 백업할 데이터가 없습니다.', 'warning');
    return;
  }
  ...
  for (const doc of snapshot.docs) {
    ...
    if (count % 500 === 0) {
      await batch.commit();
      logMigration(`진행 중: ${count}/${snapshot.size} 백업 완료`);
      batch = db.batch();
    }
  }
  ...
  logMigration(`✅ 백업 완료: ${count}개 문서`, 'success');
}

// 2단계: 데이터 변환
async function migrateData() {
  logMigration('\n========================================');
  logMigration('2단계: 주차별 → 일별 문서 변환');
  logMigration('========================================\n');
  
  const snapshot = await db.collection('schedules').get();
  logMigration(`변환할 주차별 문서: ${snapshot.size}개`);
  ...
  for (const doc of snapshot.docs) {
    ...
    if (!match) {
      logMigration(`⚠️ 문서 ID 형식 불일치, 건너뜀: ${docId}`, 'warning');
      continue;
    }
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (totalDocs % 500 === 0) {
        await batch.commit();
        logMigration(`진행 중: ${totalDocs}개 일별 문서 생성 완료`);
        batch = db.batch();
      }
    }
  }
  ...
  logMigration(`✅ 변환 완료: ${totalWorkDays}개 근무일 → ${totalDocs}개 일별 문서`, 'success');
...
async function validateData() {
  ...
  logMigration(`원본 근무일 수: ${originalWorkDays}개`);
  logMigration(`변환 문서 수: ${newDocs}개`);
  
  const isValid = originalWorkDays === newDocs;
  
  if (isValid) {
    logMigration('✅ 검증 성공: 데이터 개수 일치', 'success');
  } else {
    logMigration('❌ 검증 실패: 데이터 개수 불일치', 'error');
    logMigration(`차이: ${Math.abs(originalWorkDays - newDocs)}개`, 'error');
  }
...
async function switchCollections() {
  logMigration('\n========================================');
  logMigration('4단계: 컬렉션 전환 (schedules → schedules_old, schedules_new → schedules)');
  logMigration('========================================\n');
  
  // 기존 schedules → schedules_old
  logMigration('기존 schedules를 schedules_old로 이동 중...');
  ...
  for (const doc of oldSnapshot.docs) {
    ...
    if (count % 500 === 0) {
      await batch.commit();
      logMigration(`진행 중: ${count}개 이동 완료`);
      batch = db.batch();
    }
  }
  ...
  logMigration(`✅ schedules → schedules_old 완료: ${count}개`, 'success');
  ...
  logMigration(`✅ schedules_new → schedules 완료: ${count}개`, 'success');
  logMigration('✅ 컬렉션 전환 완료!', 'success');
}

// 메인 실행 함수
async function runMigration() {
  try {
    // firebase와 db가 정의되어 있는지 확인
    if (typeof firebase === 'undefined') {
      console.error('❌ Firebase가 로드되지 않았습니다. admin-dashboard.html에서 실행하세요.');
      return;
    }
    
    if (typeof db === 'undefined') {
      console.error('❌ Firestore db 객체가 없습니다. 페이지를 새로고침하세요.');
      return;
    }
    ...
    logMigration('========================================');
    logMigration('🚀 Schedules Collection 마이그레이션 시작');
    logMigration('========================================\n');
    logMigration(`현재 로그인: ${user.email}`);
    logMigration(`사용자 권한: ${userDoc.data().role}`);
    logMigration('✅ 관리자 권한 확인 완료\n', 'success');
    ...
    if (!structureCheck.needsMigration) {
      ...
      if (structureCheck.type === 'daily') {
        logMigration('이미 날짜별 구조로 되어있습니다.', 'success');
      }
      return;
    }
    ...
    if (!validation.isValid) {
      ...
      logMigration('schedules_backup에서 복구할 수 있습니다.', 'warning');
      return;
    }
    ...
    // 완료
    logMigration('\n========================================');
    logMigration('✅ 마이그레이션 완료!');
    logMigration('========================================\n');
    logMigration('백업 컬렉션:', 'info');
    logMigration('- schedules_backup: 원본 백업', 'info');
    logMigration('- schedules_old: 이전 주차별 문서', 'info');
...
console.log('%c🚀 마이그레이션 스크립트 로드 완료!', 'color: #28a745; font-weight: bold;');
console.log('%c========================================', 'color: #007bff; font-weight: bold;');
console.log('%c\n실행 방법:', 'color: #007bff; font-weight: bold;');
console.log('%crunMigration()', 'color: #28a745; font-size: 16px; font-weight: bold;');
console.log('%c\n위 명령어를 콘솔에 입력하고 Enter를 누르세요.\n', 'color: #666;');
