...
**버전**: v0.17.0 (대수술 완료)  
**최종 업데이트**: 2025-12-31  
**상태**: ✅ Production Ready (서버 중심 아키텍처 완료)

## 🔥 **v0.17.0 주요 변경사항**

### **서버 사이드 급여 계산 엔진 도입**
- 급여 계산은 오직 Cloud Functions에서만 수행 (14단계 파이프라인)
- 클라이언트는 서버 결과를 표시만 (보안 + 데이터 일관성)

### **데이터 무결성 강화**
- parseMoney: NaN/콤마 방지 (500 에러 차단)
- sanitizeTimestamps: React Error #31 방지
- 표준 필드명 전면 적용 (salaryAmount, clockIn, userId)

### **클라이언트 경량화**
- lib/utils/salary-calculator.ts: 800줄 → 186줄 (-75%)
- 계산 로직 완전 제거, 타입 정의만 유지

### **데이터 마이그레이션 완료**
- scripts/clean-db.ts: 데이터 정화 스크립트
- 25개 문서 정화 완료 (5.54초, 오류 0건)
...
3. ✅ **출퇴근 관리**: 출퇴근 기록, 위치 정보, QR 코드 체크인 (고정 QR - 만료 없음) 🆕
...
9. ✅ **브랜드 관리**: 브랜드 생성, 로고 업로드 (자동 압축), 색상 관리 🆕
10. ✅ **매장 관리**: 매장 생성, 급여 설정, 수당 옵션, QR 코드 통합 관리 🆕
11. ✅ **관리자 목록**: 권한 관리
...
2. ✅ **출퇴근**: 출근 버튼 → QR 스캔 → 출근 완료 (고정 QR 코드, 만료 없음) 🆕
3. ✅ **급여**: 월별 급여 조회, 명세서 다운로드
4. ✅ **스케줄**: 주간 캘린더, 매장 전체 보기 토글, 실시간 출퇴근 기록 표시 🆕
...
- ✅ **스케줄 자동 생성 시스템**: 계약서 작성 시 한 달치 스케줄 자동 생성 🆕
  - 추가 계약서 병합 지원 (같은 날 여러 시간대 표시)
  - 실시간 출퇴근 기록 자동 반영
  - 지각/조기퇴근/연장근무 상태 추적
- ✅ **직원 페이지 스케줄 탭**: 본인 스케줄 + 매장 전체 보기 토글 🆕
- ✅ **출퇴근 탭 통계 수정**: 날짜 단위 중복 제거 (하루 7번 체크해도 1일로 계산) 🆕
- ✅ **React Error #31 수정**: Firestore Timestamp 렌더링 에러 해결 🆕
- ✅ **QR 중복 생성 버그 수정**: addDoc → setDoc 결정적 ID 사용 (userId_date_time 형식)
- ✅ **허용시간 초과 시 이유 입력 강제**: 조기출근/지각/조기퇴근/연장근무 감지 시 이유 입력 모달 표시
- ✅ **Firestore 보안 규칙 강화**: schedules 컬렉션 매장 전체 조회 권한 추가
...
- 🔐 **Storage Rules v2.0**: 브랜드 로고 경로 개선 (`/brands/{companyId}/{fileName}`)
- 🔧 **매장 모달 통합**: QR 코드 관리를 매장 모달 안에 통합
- 🔐 긴급 보안 구멍 7개 수정, Next.js 15 업그레이드, Cloud Functions 구조 추가  
