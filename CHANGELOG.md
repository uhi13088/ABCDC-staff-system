...
이번 릴리스는 **Jest 기반 자동화 테스트 환경 구축**에 집중했습니다.

#### ✨ 새로운 기능

**테스트 프레임워크**:
- ✅ Jest + Babel 테스트 환경 구성
- ✅ Pure Function 테스트 67개 작성 (100% 통과)
- ✅ Firebase Mock 환경 구축
- ✅ 테스트 커버리지 86% 달성

**테스트 대상 모듈**:
- `js/salary-calculator.js` (84.51% 커버리지)
  - 급여 계산 로직 (시급/월급/연봉제)
  - 수당 계산 (야간/연장/휴일/주휴)
  - 4대보험 공제 계산
- `js/employee-utils.js` (95.34% 커버리지)
  - 시간 변환 및 계산
  - 출근 상태 처리
  - 날짜 포맷팅
...
- **신규**: [TESTING.md](TESTING.md) - 테스트 실행 및 작성 가이드
- **업데이트**: README.md - 테스트 섹션 추가
...
├── salary-calculator.test.js                 # 29개 Pure Function 테스트
├── salary-calculator-monthly.test.js         # 5개 Firebase Mock 테스트
└── employee.test.js                          # 33개 employee-utils 테스트
```

#### ⚙️ 설정 파일

- `jest.config.js` - Jest 설정 (커버리지 임계값 85%)
- `.babelrc` - ES6 → CommonJS 변환
- `.gitignore` - coverage/ 폴더 제외
